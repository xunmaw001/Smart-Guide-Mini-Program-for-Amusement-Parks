...
export default {
  ...
  methods: {
	...
	this.ruleForm.luxianxiangqing = this.ruleForm.luxianxiangqing.replace(reg,'../../../ssm58889/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `luxianjieshao/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.luxianjieshaoCrossAddOrUpdateFlag = false;
						 this.parent.search();
