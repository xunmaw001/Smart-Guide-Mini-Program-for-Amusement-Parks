	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discusspiaowuxinxi`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discusspiaowuxinxi`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discusspiaowuxinxi`, this.ruleForm);
						}else{
							await this.$api.add(`discusspiaowuxinxi`, this.ruleForm);
						}
						uni.setStorageSync('discusspiaowuxinxiCleanType',true);
