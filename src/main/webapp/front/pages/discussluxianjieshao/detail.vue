...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussluxianjieshaoCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussluxianjieshaoCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussluxianjieshao');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','discussluxianjieshao');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('discussluxianjieshao', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussluxianjieshao', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscussluxianjieshao/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('discussluxianjieshao', this.detail);
				this.$utils.msg('审核成功');
