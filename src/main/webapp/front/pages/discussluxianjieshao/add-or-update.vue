	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussluxianjieshao`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussluxianjieshao`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discussluxianjieshao`, this.ruleForm);
						}else{
							await this.$api.add(`discussluxianjieshao`, this.ruleForm);
						}
						uni.setStorageSync('discussluxianjieshaoCleanType',true);
